...
class SIREConvBase(nn.Module):
    ...
    def message_func(self, edges):
        ...
        message = F.glu(message)                                        # Gating before aggregation
...
class FinSIRModel(nn.Module):
    def __init__(self, input_dim, wiki_dim, industry_dim, correlation_dim, hidden_dim, output_dim, 
                 recurrent_layers, recurrent_dropout, relational_agg, readout_layers, readout_dropout):
        super(FinSIRModel, self).__init__()
        self.wiki_encoder = nn.Linear(wiki_dim, 2 * input_dim, bias=False)
        self.wiki_message = SIREConvBase(MessageFunction(2 * input_dim, 2 * hidden_dim, recurrent_layers, recurrent_dropout), relational_agg)
        self.industry_encoder = nn.Linear(industry_dim, 2 * input_dim, bias=False)
        self.industry_message = SIREConvBase(MessageFunction(2 * input_dim, 2 * hidden_dim, recurrent_layers, recurrent_dropout), relational_agg)
        self.correlation_encoder = nn.Linear(correlation_dim, 2 * input_dim, bias=False)
        self.correlation_message = SIREConvBase(MessageFunction(2 * input_dim, 2 * hidden_dim, recurrent_layers, recurrent_dropout), relational_agg)
        self.residual_message = MessageFunction(input_dim + 3 * hidden_dim, hidden_dim, recurrent_layers, recurrent_dropout)
        self.residual_attn = TemporalAttention(hidden_dim)
    ...
    def forward(self, wiki_graph, industry_graph, correlation_graph, stock_features):
        ...
        stock_returns = torch.cat([stock_features, wiki_returns, industry_returns, correlation_returns], dim=-1)
        stock_returns = self.residual_message(stock_returns)
        stock_returns = self.residual_attn(stock_returns)
