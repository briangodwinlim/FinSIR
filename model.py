...
class FinSIRModel(nn.Module):
    def __init__(self, input_dim, wiki_dim, industry_dim, correlation_dim, hidden_dim, output_dim, 
        ...
        self.wiki_encoder = nn.Linear(wiki_dim, 2 * hidden_dim, bias=False)
        self.wiki_message = SIREConvBase(MessageFunction(2 * hidden_dim, (1 + int(relational_agg == 'gated')) * hidden_dim, recurrent_layers, recurrent_dropout), relational_agg)
        self.industry_encoder = nn.Linear(industry_dim, 2 * hidden_dim, bias=False)
        self.industry_message = SIREConvBase(MessageFunction(2 * hidden_dim, (1 + int(relational_agg == 'gated')) * hidden_dim, recurrent_layers, recurrent_dropout), relational_agg)
        self.correlation_encoder = nn.Linear(correlation_dim, 2 * hidden_dim, bias=False)
        self.correlation_message = SIREConvBase(MessageFunction(2 * hidden_dim, (1 + int(relational_agg == 'gated')) * hidden_dim, recurrent_layers, recurrent_dropout), relational_agg)
        self.residual_func = MessageFunction(4 * hidden_dim, hidden_dim, recurrent_layers, recurrent_dropout)
    ...
    def forward(self, wiki_graph, industry_graph, correlation_graph, stock_features):
        ...
        wiki_returns = self.wiki_message(wiki_graph, stock_returns, self.wiki_encoder(wiki_graph.edata['feat']))
        industry_returns = self.industry_message(industry_graph, stock_returns, self.industry_encoder(industry_graph.edata['feat']))
        correlation_returns = self.correlation_message(correlation_graph, stock_returns, self.correlation_encoder(correlation_graph.edata['feat']))
